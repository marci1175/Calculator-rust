...
impl Engine {
            ...
            match Engine::mathdecide(&item) {
                ...
                Err(_expr) => self.expr_list.push(
                    match item.as_str() {
                        ...
                        "/" | "%" | ":" => Math::Divide,
                        "^" | "pow" => Math::Power,         
                        _ => {/*Go apeshit*/ self.invalid_equation(Error::msg("Syntax Error (Invalid expression)"), item.clone()); Math::KurvaAnyad},               
                    }
                ),
            }
        }
    ...
    fn mathengine(&mut self) -> f64 {
        let mut expr_list_copy = self.expr_list.clone();
        ...
        if self.num_list.len() > 1 {
            self.invalid_equation(Error::msg("Syntax Error (Invalid equation)"), self.num_list.iter().map(|f| format!("{} ", f).to_string()).collect());
        }
